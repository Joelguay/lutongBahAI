...
function App() {
  ...
  const handleShowRecipes = async () => {
    ...
    try {
      ...
      if (data.recipes && data.recipes.length > 0) {
        ...
        throw new Error(
          "No recipes returned. Please try again or check the backend logs."
        );
      }
    } catch (e) {
  ...
  const renderContent = () => {
    if (loading) {
      return (
        <p className="loading-text">Finding the best recipes for you...</p>
      );
    }
    ...
    if (error) {
      return (
        <div style={{ textAlign: "center" }}>
          ...
          <button className="random-button" onClick={() => setView("camera")}>
            Try Again
          </button>
        </div>
      );
